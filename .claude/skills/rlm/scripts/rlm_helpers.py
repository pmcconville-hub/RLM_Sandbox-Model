#!/usr/bin/env python3
"""Persistent mini-REPL with exploration helpers for RLM workflows.

Provides a stateful Python environment for exploring large context files
before (or instead of) running them through the full RLM pipeline.

This is a local-only tool for dev-time exploration. For production data
processing with sandbox isolation, use Mode 2 (automatic) in the /rlm skill.

Adapted from Brainqub3/claude_code_RLM with additions for RLM_Sandbox-Model.

Usage:
  python rlm_helpers.py init path/to/context.txt
  python rlm_helpers.py status
  python rlm_helpers.py exec -c 'print(len(content))'
  python rlm_helpers.py exec -c 'print(peek(0, 2000))'
  python rlm_helpers.py exec -c 'hits = grep("TODO"); print(hits[:3])'
  python rlm_helpers.py export-buffers results.txt
  python rlm_helpers.py reset
...
class RlmHelpersError(RuntimeError):
    pass
...
def _load_state(state_path: Path) -> Dict[str, Any]:
    if not state_path.exists():
        raise RlmHelpersError(
            f"No state found at {state_path}. Run: python rlm_helpers.py init <context_path>"
        )
    with state_path.open("rb") as f:
        state = pickle.load(f)
    if not isinstance(state, dict):
        raise RlmHelpersError(f"Corrupt state file: {state_path}")
    return state
...
def _read_text_file(path: Path, max_bytes: int | None = None) -> str:
    if not path.exists():
        raise RlmHelpersError(f"Context file does not exist: {path}")
...
def _make_helpers(context_ref: Dict[str, Any], buffers_ref: List[str]):
    def peek(start: int = 0, end: int = 1000) -> str:
        """Return a slice of the loaded context content."""
    ...
    def grep(
        ...
        """Search the loaded context for a regex pattern."""
        content = context_ref.get("content", "")
    ...
    def chunk_indices(size: int = 200_000, overlap: int = 0) -> List[Tuple[int, int]]:
        """Return (start, end) index pairs for chunking the context."""
        if size <= 0:
    ...
    def write_chunks(
        ...
        """Write context chunks to individual files. Returns list of file paths."""
        content = context_ref.get("content", "")
    ...
    def add_buffer(text: str) -> None:
        """Append a text result to the buffers list for later synthesis."""
...
def cmd_init(args: argparse.Namespace) -> int:
    ...
    print(f"Initialised RLM helpers state at: {state_path}")
...
def cmd_exec(args: argparse.Namespace) -> int:
    ...
    if not isinstance(ctx, dict) or "content" not in ctx:
        raise RlmHelpersError("State is missing a valid 'context'. Re-run init.")
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        ...
        description=textwrap.dedent(
            ...
            Examples:
              python rlm_helpers.py init context.txt
              python rlm_helpers.py status
              python rlm_helpers.py exec -c "print(len(content))"
              python rlm_helpers.py exec -c "print(peek(0, 2000))"
              python rlm_helpers.py exec -c "hits = grep('TODO'); print(hits[:3])"
